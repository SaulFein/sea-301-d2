...
Article.allAuthors = function() {
  return Article.all.map(function(article){
    return article.author;                    //outputs author strings in new array
  })
  .reduce(function(prev, cur) { //input to reduce array of authors
    if(prev.indexOf(cur) < 0){
      prev.push(cur);
    }
      return prev;
    }, []); //the functions is the callback function and the [] initial vaule
};
